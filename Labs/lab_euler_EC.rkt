...
(define (helper G A)
  (if (null? G)
      (usort A)
      (helper (cdr G) (append (list(car(car G))) (cdr(car G)) A))
      )
  )
...
(define (helper1 G A)
  (if (null? G)
      A
      (helper1 (cdr G) (append (list(car(car G))) (cdr(car G)) A))
      )
  )

(define (helper2 V A C)
  (if (null? A)
      C
      (if (equal? (car A) V)
          (helper2 V (cdr A) (+ 1 C))
...
(define (helper4 G V A)
  (if (null? G)
      (usort A)
      (if (equal? V (car(car G)))
          (helper4 (cdr G) V (append (cdr(car G)) A))
          (if (equal? V (car(cdr(car G))))
              (helper4 (cdr G) V (append (list (car(car G))) A))
              (helper4 (cdr G) V A)
              )
          )
      )
...
(define (connected? G)
  (if (null? G)
      #t
      (helper5 G (get-vertices G) (get-vertices G))
      )
  )

(define (helper5 G L V)
  (if (null? V)
      #t
      (if (equal? L (usort (dfs G (car V))))
          (helper5 G L (cdr V))
          #f
          )
      )
