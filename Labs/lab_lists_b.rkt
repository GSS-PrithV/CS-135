...
#|-------------------------------------------------------------------------------
 ...
 | Pledge: I pledge my honor that I have abided by the Stevens onor System.
 |-------------------------------------------------------------------------------|#
...
;; 2 PTS
(define (yoda words)(cons (car (cdr (cdr words))) (reverse (cdr (reverse words)))))
...
(define (rotate c)
        (list
             (list (car c) (car (cdr c)))
             (list (car (cdr c)) (car c))
             (list (-(car c)) (car (cdr c)))
             (list (-(car (cdr c))) (car c))
             )
   )
...
(define (get-streetname b)
  (list 
   (car(cdr(car(car(cdr(cdr(cdr(cdr b))))))))
  )
...
(define (combine-products b1 b2)
   (append
   (car (cdr( cdr( cdr( cdr( cdr b1 ))))))
   (car (cdr( cdr( cdr( cdr( cdr b2 ))))))
   )
