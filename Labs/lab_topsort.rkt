...
;; 5 PTS
(define (find-top G)
   (helper2 G (get-vertices G))
)

(define (helper2 L v)
  (if (null? v)
      (topsort-error)
      (if (zero? (indegree (get-edges L) (car v)))
      (car v)
      (helper2 L (cdr v))
      )
  )
...
(define(helper3 v V E L)
  (if (null? V)
  (list (reverse (append  V L)) E)
  (if (equal? v (car V))
      (list (reverse (append (cdr V) L)) E)
      (helper3 v (cdr V) E (append (list (car V)) L))
      )
  )
  )
...
;; 5 PTS
(define (topsort G)
  (helper4 G (get-vertices G) '())
  )

(define (helper4 G V L)
  (if (null? V)
      L
      (helper4 (pop-vertex G (find-top G)) (cdr V) (append L (list (find-top G))))
      )
  )

(define (topsort-2 G)
  (helper5 G (get-vertices G))
  )

(define (helper5 G V)
  (if (null? V) '()
      (cons (find-top G) (helper5 (pop-vertex G (find-top G)) (cdr V))
  )
      )
  )