...
#|-------------------------------------------------------------------------------
 | Name: Prithvinarayan Revuri
 | Pledge: I pledge my honor that I have abided by the Stevens Honor system
 |-------------------------------------------------------------------------------|#

(helper3 (helper1 G '()) 0 (length G))
(define (helper3 G A L)
  (if (null? G)
      (if (>= (* 0.5 L) (* 1.0 A))
          #t
          A
...
#(define (helper cA cB g fA fB)
  (cond
    [(equal? fB g) '(fA g)]
    [(equal? fB cB) (list '(fA fB) (helper cA cB g fA 0))]
    [(zero? fA) (list '(fA fB) (helper cA cB g cA fB))]
    [(> (+ fA fB) cB) (list '(fA fB) (helper cA cB g (- cB (+ fA fB)) cB))]
    [(<= (+ fA fB) cB) (list '(fA fB) (helper cA cB g 0 (+ fA fB)))]
    )
  )

(define (helper cA cB g fA fB L)
  (cond
    [(equal? fB g) (reverse (append (list (list fA g)) L))]
    [(equal? fB cB) (helper cA cB g fA 0 (append (list (list fA fB)) L))]
    [(zero? fA) (helper cA cB g cA fB (append (list (list fA fB)) L))]
    [(> (+ fA fB) cB) (helper cA cB g (- (+ fA fB) cB) cB (append (list (list fA fB)) L))]
    [(<= (+ fA fB) cB) (helper cA cB g 0 (+ fA fB) (append (list (list fA fB)) L))]
    )
  )
...
#(define (helper cA cB g fA fB)
  (if (equal? fB g)
      '(fA,fB)
      (if (equal? fB cB)
          (list '(fA 0) (helper cA cB g fA 0))
          (if (zero? fA)
              (list '(cA fB) (helper cA cB g cA fB))
              (if (> (+ fA fB) cB)
                  (list '(- cB (+ fA fB)) (helper cA cB g (- cB (+ fA fB)) cB))
                  (list '(0 (+ fA fB)) (helper cA cB g 0 (+ fA fB)))
              )
          )
      )
  )
